...
$(document).ready(function () {
    ...
    $("#fetchStatementForm").validate({
        ...
        messages: {date_to: 'date difference should not be more than a year'}
    });
...
function dtInit()
{
    ...
    var t4 = $('#statementTable').DataTable({
        lengthMenu: [[ -1], [ 'All']],
        paging: false,
        info: false,
        searching: false
...
function getDashboardData()
{
    $.ajax({
        type: "post",
        url: "/bursarhome/getdashboarddata",
        data: {},
        dataType: "json",
                ...
                columnDefs: [ {
                  ...
                  footerCallback: function ( row, data, start, end, display ) {
                    ...
                    var balance;
                        ...
                        var pageTotal = api
                    .column( 2, { page: 'allPages'} )
                    .data()
                    .reduce( function (a, b) {
                        return Number(a) + Number(b);
                    }, 0 );
                    // Update footer
                    $( api.column( 2 ).footer() ).html(getCurrency(pageTotal));  
                }                  
            });
