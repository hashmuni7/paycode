...
function printSchoolFeesBalances()
{
    ...
    doc.save('OutstandingBalance_'+ $('#balanceSubHead').html() +  displayDate(new Date())  +'.pdf');
...
{   
    ...
    $.ajax({
        type: "post",
        url: "/bursarhome/finance/schoolfees/classbalances/getoutstandingbalance",
        data: {
            ...
            $('#balanceSubHead').html($('#classes :selected').text() + ' ' + stream + ' ' + $('#terms :selected').text())
             ...
             var t = $("#schoolFeesBalancesTable").DataTable({
                ...
                footerCallback: function ( row, data, start, end, display ) {
                    ...
                    // Update footer
                    $( api.column( 3 ).footer() ).html(getCurrency(pageTotal));                                                   
                         j++;
                    }
                } 
