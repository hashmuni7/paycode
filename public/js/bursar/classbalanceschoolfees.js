...
$(document).ready(function () {
    ...
    $('#fetchBalancesBtn').on('click', function (e) {
        ...
        {
            $('#printSchoolFeesBalances').addClass('hidden');
            $('#printSchoolFeesBalancesB').addClass('hidden');
            getStreamTermFeesBalance();
    ...
    // Click event for the print 
    $('#printSchoolFeesBalancesB').click(function (e) {
        if(e.detail !== 2)
        {       
                         
            printSchoolFeesBalancesB();
        }         
...
function printSchoolFeesBalances()
{
    ...
    doc.save('SchoolFeesBalance_'+ $('#balanceSubHead').html() +  displayDate(new Date())  +'.pdf');
    
}


function printSchoolFeesBalancesB()
{
    

    var tbl = $('#schoolFeesBalancesTable').clone();
    tbl.find(
'tr th:nth-child(4), tr td:nth-child(4), tr th:nth-child(5), tr td:nth-child(5)').remove();
    // Put footer at the end
    var foot = tbl.find('tfoot');
    var last = foot.find('tr');
    tbl.find('tfoot').remove();
...
{   
    ...
    $.ajax({
        type: "post",
        url: "/bursarhome/finance/schoolfees/classbalances/getbalanceofstream",
        data: {
        ...
        dataType: "json",
        success: function (response, status, code) {
            $('#printSchoolFeesBalances').removeClass('hidden');
            $('#printSchoolFeesBalancesB').removeClass('hidden');
...
{   
    ...
    $.ajax({
        type: "post",
        url: "/bursarhome/finance/schoolfees/classbalances/getbalanceofsection",
        data: {
            section_id: section,
            term_id: term
        },            
