    ...
    $.each(subjectsDoneByClass, function(key, val){
        ...
        if(val.subjectbyclassid == subjectbyclassid )
        {
            ...
            if(isChecked)
            {
                ...
                subjects_arr.push(parseInt(subjectbyclassid)); // Also put subject in subjects of exam in an organised order
            }
            else{
...
{        
    ...
    //     function(){
    //         subjects_arr.push(parseInt($(this).val()));
    //     }
    // );
    ...
    $('.exam-subjects-agg input:checked').each(
     ...
     $.ajax({
        type: "post",
        url: "/doshome/gradesandacademics/exams/createexam", 
        data: { 
...
function goToExam(examid)
{
    //alert('The Exam is ready');
     var url = '/orms/public/doshome/gradesandacademics/exams/all';
