...
$(document).ready(function () {
    ...
    $('#termlyFeesAttached').on('click', '.edit-termly-fees-btn', function (e) {
        e.preventDefault();
        //aRow = $(this).closest('tr');
        //fetchMethod($(this).attr('data-id'));
        fetchTermlySchoolFees($(this).attr('data-termhistoryid'), $(this).attr('data-term'), $(this).attr('data-termfeesamount'));
    });
    $('#termlyFeesAttached').on('click', '.delete-termly-fees-btn', function (e) {
        e.preventDefault();
        askDeletePupilTerm($(this).attr('data-pupilid'), $(this).attr('data-termid'), $(this).attr('data-term'));
    ...
    // Click event for the update method
    $('#updatetermlySchoolFeesBtn').click(function (e) { 
        if(e.detail !== 2)
        {                                
            updateTermlySchoolFees();
        }         
	...
	*/
	...
	Modal Dismiss
	*/
	$(document).on('click', '.modal-dismiss-termlyschoolfees', function (e) {
		e.preventDefault();
        resetTermlySchoolFeesModal();
        $.magnificPopup.close();
    });
	...
	*/
    ...
    $('#printtermlyFeeAttachment').click(function (e) { 
        if(e.detail !== 2)
        {       
                         
            printtermlyFeeAttachment();
        }
    });// end click event
...
function printtermlyFeeAttachment()
{
    var tbl = $('#termlyFeesAttached').clone();
    ...
    doc.text(schoolName, center, schoolNameY, 'center');   
    doc.setFontSize(16);
    doc.text($('#feesAttachedHead2').html(), center, 32, 'center');
    doc.setDrawColor(headColor[0], headColor[1], headColor[2]);
    doc.setLineWidth(1);
    doc.line(15, 36, 195, 36);
    doc.setFontSize(11);   
    ...
    doc.save($('#feesAttachedHead2').html() + '_'+ displayDate(new Date())  +'.pdf');
}
...
function dtInit()
{
   ...
   // theTable = t;
    ...
    var ft =$('#termlyFeesAttached').DataTable({
        lengthMenu: [[ 50, 100, 200, -1], [ 50, 100, 200, 'All']]
    });
    theTable = ft;
    ft.on( 'order.dt search.dt', function () {
        ft.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
            cell.innerHTML = i+1;
        } );
    } ).draw();
    ...
    if($('#addPaymentsForm').valid()) // Test if all Form Fields are validated
        {
            ...
            $.ajax({
                type: "post",
                url: "/adminhome/finance/schoolfees/addpayment", 
                data: { 
    ...
    if($('#addaccountsPaymentsForm').valid()) // Test if all Form Fields are validated
        {
            if($('#localAccounts').val() != '')
            {
                ...
                $.ajax({
                    type: "post",
                    url: "/adminhome/finance/localaccounts/addpayment", 
                    data: { 
    ...
    if($('#addactivitiesPaymentsForm').valid()) // Test if all Form Fields are validated
        {
            if($('#activities').val() != '')
            {
                ...
                $.ajax({
                    type: "post",
                    url: "/adminhome/finance/activities/addactivitypayment", 
                    data: { 
...
function resetTermlySchoolFeesModal()
{
    document.getElementById('termlySchoolFeesForm').reset();
}

function resetTermlySchoolFeesPaymentsModal()
{
    $('#addSchoolFeesPaymentBtn222').show();
    $('#updateSchoolFeesPaymentBtn222').addClass('hidden');
    
    
    document.getElementById('addPaymentsForm222').reset();
...
function fetchSchoolFeesPayment(paymentid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/schoolfeespaymentbyid",
...
function fetchTermlySchoolFees(termhistoryid, term, amount)
{
    // $('#schoolFeesForm').trigger('loading-overlay:show');
    // $('#addSchoolFeesPaymentBtn').hide();// Hide the register Button and show Update Btn
    // $('#updateSchoolFeesPaymentBtn').removeClass('hidden');
    ...
    $('#termhistoryid').val(termhistoryid)
    $('#termlyFeesTerm').val(term);
    $('#termlyFeesAmount').val(amount);    
...
function fetchLocalAccountPayment(receiptid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/localaccounts/getreceiptbyid",
...
function fetchActivityPayment(activityreceiptid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/activities/getactivitypaymentbyid",
...
{   
        ...
        if($('#addPaymentsForm').valid()) // Test if all Form Fields are validated
        {
            ...
            $.ajax({
                type: "post",
                url: "/adminhome/finance/payments/updateschoolfeespayment",
                data: {
...
// Update the Method Details By its id
// Get the field to update from the input field
function updateTermlySchoolFees()
{   
    
     
        if($('#termlySchoolFeesForm').valid()) // Test if all Form Fields are validated
        {
            
            $.ajax({
                type: "post",
                url: "/adminhome/finance/payments/schoolfees/updatepupiltermlyfees",
                data: {
                    termhistory_id: parseInt($('#termhistoryid').val()), 
                    amount: parseInt($('#termlyFeesAmount').val()),                    
                },
                dataType: "json",
                success: function (response, status, code) {
...
{   
        ...
        if($('#addaccountsPaymentsForm').valid()) // Test if all Form Fields are validated
        {
            ...
            $.ajax({
                type: "post",
                url: "/adminhome/finance/localaccounts/updatereceipt",
                data: {                
...
{   
        ...
        if($('#addactivitiesPaymentsForm').valid()) // Test if all Form Fields are validated
        {
            ...
            $.ajax({
                type: "post",
                url: "/adminhome/finance/activities/updateactivitypayment",
                data: {                
...
function getPupilSchoolFeesPayments()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/getpupilschoolfeespayments/" + pupilid,
        data: {},            
        dataType: "json",
        success: function (response, status, code) {
...
function getPupilLocalAccountsPayments()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/localaccounts/getpupilreceiptpayments/" + pupilid,
        data: {},            
        dataType: "json",
        success: function (response, status, code) {
            // First destroy the DataTable Before creating a new one
...
function getPupilActivityPayments()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/activities/getpupilactivitypayments/" + pupilid,
        data: {},            
        dataType: "json",
        success: function (response, status, code) {
            // First destroy the DataTable Before creating a new one
...
function getPupilAllTermsFeesBalance()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/schoolfees/getpupilbalance",
        data: {
            pupil_id: pupilid
        },            
        dataType: "json",
...
function getPupilTermlyFees()
{
    ...
    $.ajax({
        ...
        success: function (response, status, code) {
            ...
            var stringed = JSON.stringify(response.data.data)
            // First destroy the DataTable Before creating a new one
            $('#termlyFeesAttached').DataTable().destroy();
            
             // Create a new dataTable and assign it to a variable t
             var t = $("#termlyFeesAttached").DataTable({
                data: JSON.parse(stringed),
                columns: [
                    { data: null },
                    { data: 'term'},
                ...
                columnDefs: [ {
                    ...
                    data: 'termhistoryid',
                    className: 'actions-hover actions-fade',
                    render: function (r, f, data, m) {
                                    
                                    return '<a class="ws-normal context-menu pointer edit-termly-fees-btn"  data-termhistoryid="'+ data.termhistoryid + '" data-term="'+ data.term + '" data-termfeesamount="'+ data.amount + '">'+ 
                                    '<i class="fas fa-pencil-alt"></i> Edit</a>' + 
                                    '<a class="ws-normal context-menu pointer delete-termly-fees-btn"  data-termhistoryid="'+ data.termhistoryid + '" data-term="'+ data.term + '" data-termfeesamount="'+ data.amount + '" data-pupilid="'+ data.pupilid + '" data-termid="'+ data.termid + '">' +
                                    '<i class="fas fa-trash-alt"></i> Delete Term</a>'
                                }
                  } ],
                  lengthMenu: [[ 50, 100, 200, -1], [ 50, 100, 200, 'All']]
...
function getPupilAllTermsLocalAccountsBalance()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/localaccounts/getpupilbalance",
        data: {
            pupil_id: pupilid
        },            
        dataType: "json",
...
function getPupilAllTermsActivitiesBalance()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/activities/getpupilbalance",
        data: {
            pupil_id: pupilid
        },            
        dataType: "json",
...
function getPupilSchoolFeesPaymentsForTerm()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/schoolfees/getpupiltermsummary",
        data: {
            pupil_id: pupilid,
...
function getPupilLocalAccountsPaymentsForTerm()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/localaccounts/getpupiltermsummary",
        data: {
            pupil_id: pupilid,
            term_id: parseInt($('#summaryTerms').val())
        },            
...
function getPupilActivityPaymentsForTerm()
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/activities/getpupiltermsummary",
        data: {
            pupil_id: pupilid,
            term_id: parseInt($('#summaryTerms').val())
        },            
...
function deleteSchoolFeesPayment(paymentid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/deletepupilschoolfeespayments",
        data: { payment_id: paymentid},
        dataType: "json",
        success: function (response, status, code) {
...
// Pop up a confirm dialog to delete a method
function askDeletePupilTerm(pupilid, termid, term)
{
    $.alert({
        title: 'Are You Sure You Want to Delete the Selected Term',
        content: $('#pupilid').val() + ' will no longer have ' + term,       
        buttons: {
            confirm: function (){
               deletePupilTerm(pupilid, termid, term);
            },
            cancel: function (){}
        },
        icon: 'fa fa-spinner fa-spin',
...
function deletePupilTerm(pupilid, termid, term)
{
    
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/schoolfees/deletepupilterm",
        data: { 
...
function deleteLocalAccountsPayment(receiptid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/localaccounts/deletereceipt",
        data: { receipt_id: receiptid},
        dataType: "json",
        success: function (response, status, code) {
...
function deleteActivityPayment(activityreceiptid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/activities/deleteactivitypayment",
        data: { activity_receipt_id: activityreceiptid},
        dataType: "json",
        success: function (response, status, code) {
...
function detachFee(demandid)
{
    ...
    $.ajax({
        type: "post",
        url: "/adminhome/finance/payments/localaccounts/detachfee",
