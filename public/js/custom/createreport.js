	...
	*/
    ...
    $('#classes').on('change', function () {
        ...
        // ajax request to create a new class to the database
            $.ajax({
                type: "post",
                url: "/adminhome/gradesandacademics/reports/getclassexams", 
                data: { 
                    class_id: classid
                },
                dataType: "json",
                success: function (response, status, code) {
                    $('#exams').empty();
                    $('#exams').append('<option></option>');
                    $.each(response.data.data, function(key, val){
                        
                            var newItem = '<option value="' + val.examid + '">' + val.exam + '</option>';
                            $('#exams').append(newItem);
...
function createReport()
{
     ...
     $.ajax({
        type: "post",
        url: "/adminhome/gradesandacademics/reports/createreport", 
        data: { 
            name_of_report: $('#nameOfReport').val(),
            exam_id: parseInt($('#exams').val()) 
        },
        dataType: "json",
        success: function (response, status, code) {
            console.log(response);
            notifySuccess(response.data.msg);
            // var testid = response.data.data;
            goToReports();
...
// Go to all reports list
function goToReports()
{
    //alert('The Test is ready');
     var url = '/orms/public/adminhome/gradesandacademics/reports/all';
